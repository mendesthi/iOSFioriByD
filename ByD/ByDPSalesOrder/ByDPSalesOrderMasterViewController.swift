//
// ByDPSalesOrderMasterViewController.swift
// ByD
//
...
class ByDPSalesOrderMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    private let appDelegate = UIApplication.shared.delegate as! AppDelegate
    private var byDC: ByDC<OnlineODataProvider> {
        return self.appDelegate.byDC
    }

    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([ByDPSalesOrder]?, Error?) -> Void) -> Void)?
    private var entities: [ByDPSalesOrder] = [ByDPSalesOrder]()
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        self.byDC.deleteEntity(currentEntity) { error in
            if let error = error {
...
extension ByDPSalesOrderMasterViewController: EntitySetUpdaterDelegate {
